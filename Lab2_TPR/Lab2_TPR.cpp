...
int expect_large_factory(double x[14], int* i, double first_result[2])
{
	...
	cout << "Очiкування i великий завод: \t" << first_result[*i] << endl;
	*i = *i + 1;
	return 0;
}
...
int expect_small_factory(double x[14], int* i, double first_result[2])
{
	...
	cout << "Очiкування i малий завод: \t" << first_result[*i] << endl;
	*i = *i + 1;
	return 0;
}
...
int Large_factory(double x[14], int* j, double result[3])
{
	...
	cout << "Великий завод: \t\t\t" << result[*j] << endl;
	*j = *j + 1;
	return 0;
}
...
int small_factory(double x[14], int* j, double result[3])
{
	...
	cout << "Малий завод: \t\t" << result[*j] << endl;
	*j = *j + 1;
	return 0;
}
...
void main()
{
	...
	cout << endl << "Найоптимальнiшим вирiшенням є: ";
	better_all(result);
